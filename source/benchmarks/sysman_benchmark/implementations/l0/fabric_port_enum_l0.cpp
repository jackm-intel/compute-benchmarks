...
static TestResult run(const FabricPortEnumArguments &arguments, Statistics &statistics) {
    ...
    for (auto j = 0u; j < arguments.iterations; j++) {
        uint32_t count = 0;
        timer.measureStart();
        result = zesDeviceEnumFabricPorts(device, &count, nullptr);
        if (result != ZE_RESULT_SUCCESS) {
            return TestResult::DeviceNotCapable;
        }
        std::vector<zes_fabric_port_handle_t> handles(count);
        if (count > 0) {
            result = zesDeviceEnumFabricPorts(device, &count, handles.data());
            if (result != ZE_RESULT_SUCCESS) {
                return TestResult::DeviceNotCapable;
            }
        }
...
static RegisterTestCaseImplementation<FabricPortEnum> registerTestCase(run, Api::L0);
