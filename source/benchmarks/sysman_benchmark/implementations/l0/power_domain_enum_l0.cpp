...
static TestResult run(const PowerDomainEnumArguments &arguments, Statistics &statistics) {
    ...
    for (auto j = 0u; j < arguments.iterations; j++) {
        ...
        std::vector<zes_pwr_handle_t> handles(count);
        if (count > 0) {
            result = zesDeviceEnumPowerDomains(device, &count, handles.data());
            if (result != ZE_RESULT_SUCCESS) {
                return TestResult::DeviceNotCapable;
            }
        }
...
static RegisterTestCaseImplementation<PowerDomainEnum> registerTestCase(run, Api::L0);
