...
static TestResult run(const TemperatureSensorEnumArguments &arguments, Statistics &statistics) {
    ...
    for (auto j = 0u; j < arguments.iterations; j++) {
        uint32_t count = 0;
        timer.measureStart();
        result = zesDeviceEnumTemperatureSensors(device, &count, nullptr);
        if (result != ZE_RESULT_SUCCESS) {
            return TestResult::DeviceNotCapable;
        }
        std::vector<zes_temp_handle_t> handles(count);
        if (count > 0) {
            result = zesDeviceEnumTemperatureSensors(device, &count, handles.data());
            if (result != ZE_RESULT_SUCCESS) {
                return TestResult::DeviceNotCapable;
            }
        }
...
static RegisterTestCaseImplementation<TemperatureSensorEnum> registerTestCase(run, Api::L0);
